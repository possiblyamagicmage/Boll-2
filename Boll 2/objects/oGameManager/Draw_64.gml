...
if surface_exists(HUDsurface) {
	...
	var HUDlefthandle = 26;
	var HUDrighthandle = guiw-8;
	var HUDtophandle = 16;
	var yy = HUDtophandle;
	var text_offset = 2;
	var font_timer_offset = 0;
	
	var yoff = 4*(HUDcoinflash/2)
	
	var font = global.hudFont;
	
	switch(global.settings[$ "alternate_hud"]) {
		case 1:
			font = global.rulerGold;
			font_timer_offset = 2;
		break;
	}
	...
	draw_text(HUDlefthandle,yy-yoff+text_offset,$"x{_coins}");
	if (HUDcoinflash > 0.0) {
		draw_text_color(HUDlefthandle,yy-yoff+text_offset,$" {_coins}",c_yellow,c_yellow,c_yellow,c_yellow,HUDcoinflash);
	}
	
	HUDcoinflash=max(HUDcoinflash-0.1,0);
	
	//Timer
	var clock_spacing = 12;
	...
	draw_text(HUDrighthandle,yy+font_timer_offset,timestr);
	draw_set_halign(0);
	draw_set_valign(0);
	...
	repeat (array_length(global._playerChars)) {
		...
		draw_text(HUDlefthandle+32*i,yy+text_offset,$"x{_lives}");
		i++;
	}
	...
	if (reserved_item != noone) {
		...
		draw_sprite_ext(sprite,0,HUDlefthandle-4,yy,1-scale,1-scale,0,c_white,1);
